<template>
  <div class="space-y-4">
    ...
    <template v-else-if="user">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div class="flex items-start gap-5">
          ...
          <label class="cursor-pointer shrink-0 group">
            ...
            <p class="text-[10px] text-gray-400 mt-1 text-center">JPG, PNG, WebP. Макс. 2 МБ</p>
          </label>
      ...
      <!-- Редактирование -->
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h3 class="font-bold text-[#0A1F44] text-sm mb-4">Редактировать данные</h3>
        <form @submit.prevent="saveUser" class="space-y-3">
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1">ФИО</label>
            ...
            <div v-if="user.role !== 'owner'">
              <label class="block text-xs font-medium text-gray-600 mb-1">Роль</label>
      ...
      <!-- Сброс пароля -->
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h3 class="font-bold text-[#0A1F44] text-sm mb-4">Сброс пароля</h3>
        <form @submit.prevent="resetPassword" class="space-y-3">
          <div class="max-w-sm">
            <label class="block text-xs font-medium text-gray-600 mb-1">Новый пароль</label>
            <div class="relative">
          ...
          <label class="flex items-center gap-2 text-sm text-gray-500">
            ...
            Отправить новый пароль на {{ user.email }}
          </label>
          <div class="flex items-center gap-3">
            <button type="submit" :disabled="resettingPassword || newPassword.length < 8"
              ...
              {{ resettingPassword ? 'Сохраняем...' : 'Обновить пароль' }}
            </button>
      ...
      <div v-if="user.role !== 'owner'" class="bg-white rounded-2xl border border-red-100 shadow-sm p-5">
        <h3 class="font-bold text-red-600 text-sm mb-2">Удалить сотрудника</h3>
        <p class="text-xs text-gray-500 mb-3">Сотрудник будет деактивирован и удалён. Это действие нельзя отменить.</p>
  ...
  <AppModal v-model="showDeleteModal" title="Удалить сотрудника?">
    <p class="text-sm text-gray-600 mb-4">
      Вы уверены, что хотите удалить <strong>{{ user?.name }}</strong>? Это действие нельзя отменить.
    </p>
...
async function resetPassword() {
  ...
  try {
    ...
    passwordMsg.value = sendPasswordEmail.value
      ? 'Пароль обновлён и отправлен на email'
      : 'Пароль обновлён';
    newPassword.value = '';
