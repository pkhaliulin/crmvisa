    ...
    <div v-else-if="agency">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 text-center">
                ...
                <div class="text-xs text-gray-400 mt-1">{{ t('agencyDetail.totalCases') }}</div>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 text-center">
                <div class="text-2xl font-bold" :class="agency.success_rate >= 80 ? 'text-[#1BA97F]' : agency.success_rate >= 50 ? 'text-amber-500' : 'text-[#0A1F44]'">
                    {{ agency.success_rate != null ? agency.success_rate + '%' : '--' }}
                </div>
                <div class="text-xs text-gray-400 mt-1">{{ t('agencyDetail.successRate') }}</div>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 text-center">
                <div class="text-2xl font-bold text-[#0A1F44]">{{ agency.team_count || 0 }}</div>
                <div class="text-xs text-gray-400 mt-1">{{ t('agencyDetail.teamCount') }}</div>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 text-center">
                <div class="text-2xl font-bold text-[#0A1F44]">{{ agency.clients_count || 0 }}</div>
                <div class="text-xs text-gray-400 mt-1">{{ t('agencyDetail.clientsCount') }}</div>
            </div>
        </div>
